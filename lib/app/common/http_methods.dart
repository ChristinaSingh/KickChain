...
class MyHttp {
  ...
  // ✅ IMPROVED: Always return response, let caller handle errors
  static Future<http.Response?> postMethod({
    ...
    if (!await CommonWidgets.internetConnectionCheckerMethod()) {
      if (wantSnackBar) {
        CommonWidgets.snackBarView(title: "No internet connection");
      }
      return null;
    }
    ...
    try {
      ...
      if (wantSnackBar) {
        await CommonWidgets.responseCheckForPostMethodNew(
          response: response,
          wantSnackBar: true,
        );
      }

      return response; // ✅ Always return response, even on error

    } catch (e) {
      if (kDebugMode) print("❌ Exception: $e");
      if (wantSnackBar) {
        CommonWidgets.snackBarView(title: "Server error. Please try again.");
      }
      return null;
    }
  }
